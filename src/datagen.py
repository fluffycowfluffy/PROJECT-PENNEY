...
def decks_to_npy() -> None:
  ...
  # set seed based on number of files in directory
  deck_seed = max(0, len(files))
  ...
  while more_decks == "Y":
    ...
    try:
      ...
      print(f"{additional_decks} decks saved to: {deck_path} with random seed {deck_seed}")
      try: 
      ...
      print(f"Experienced error: {e}. Continuing without additional deck generation.")
    break
  if more_decks == "N":
